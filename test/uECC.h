...
/* Optimization level; trade speed for code size.
   ...
   Currently supported values are 0 - 3; 0 is unusably slow for most
   applications. */
...
/* uECC_cheap_compress() function.
Compress a (special) public key like uECC_compress() but using less space.

Inputs:
    public_key - The public key to compress. The 0th bit in the last byte of the
pubkey must be 0.

Outputs:
    compressed - Will be filled in with the compressed public key. Must be at
least (curve size) bytes long; for example, if the curve is secp256r1,
                 compressed must be 32 bytes long.
*/
void uECC_cheap_compress(const uint8_t *public_key, uint8_t *compressed, uECC_Curve curve);

/* uECC_cheap_decompress() function.
Decompress a cheap compressed public key.

Inputs:
    compressed - The cheap compressed public key.
...
update_hash(), and finally a call to finish_hash() to prudoce the resulting
hash.
...
Usage: Compute a hash of the data you wish to sign (SHA-2 is recommended) and
pass it in to this function along with your private key and a hash context. Note
