...
uECC_VLI_API uECC_word_t uECC_vli_testBit(const uECC_word_t *vli, bitcount_t bit) { return (vli[bit >> uECC_WORD_BITS_SHIFT] & ((uECC_word_t)1 << (bit & uECC_WORD_BITS_MASK))); }
...
#if uECC_ENABLE_VLI_API
uECC_VLI_API void uECC_vli_square(uECC_word_t *result, const uECC_word_t *left, wordcount_t num_words) { uECC_vli_mult(result, left, left, num_words); }
...
uECC_VLI_API void uECC_vli_modSquare_fast(uECC_word_t *result, const uECC_word_t *left, uECC_Curve curve) { uECC_vli_modMult_fast(result, left, left, curve); }
...
static uECC_word_t EccPoint_compute_public_key(uECC_word_t *result, uECC_word_t *private, uECC_Curve curve) {
...
uECC_VLI_API void uECC_vli_bytesToNative(uint8_t *native, const uint8_t *bytes, int num_bytes) { uECC_vli_nativeToBytes(native, num_bytes, bytes); }
...
int uECC_make_key(uint8_t *public_key, uint8_t *private_key, uECC_Curve curve) {
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (EccPoint_compute_public_key(public, private, curve)) {
            ...
            uECC_vli_nativeToBytes(public_key + curve->num_bytes, curve->num_bytes, public + curve->num_words);
            return 1;
        }
    }
    return 0;
...
void uECC_cheap_compress(const uint8_t *public_key, uint8_t *compressed, uECC_Curve curve) {
    wordcount_t i;
    for (i = 0; i < curve->num_bytes; ++i) {
        compressed[i] = public_key[i];
    }
    // compressed[0] = 2 + (public_key[curve->num_bytes * 2 - 1] & 0x01);
}

void uECC_cheap_decompress(const uint8_t *compressed, uint8_t *public_key, uECC_Curve curve) {
    uECC_word_t point[uECC_MAX_WORDS * 2];
    uECC_word_t *y = point + curve->num_words;
...
int uECC_valid_public_key(const uint8_t *public_key, uECC_Curve curve) {
    ...
    uECC_vli_bytesToNative(public + curve->num_words, public_key + curve->num_bytes, curve->num_bytes);
...
static void HMAC_update(const uECC_HashContext *hash_context, const uint8_t *message, unsigned message_size) { hash_context->update_hash(hash_context, message, message_size); }
...
int uECC_verify(const uint8_t *public_key, const uint8_t *message_hash, unsigned hash_size, const uint8_t *signature, uECC_Curve curve) {
    ...
    uECC_vli_bytesToNative(public, public_key, curve->num_bytes);
    uECC_vli_bytesToNative(public + num_words, public_key + curve->num_bytes, curve->num_bytes);
...
unsigned uECC_curve_num_n_words(uECC_Curve curve) { return BITS_TO_WORDS(curve->num_n_bits); }

unsigned uECC_curve_num_n_bytes(uECC_Curve curve) { return BITS_TO_BYTES(curve->num_n_bits); }
